	class Process {
		constructor() {
			...
			this.resize();
			//listenes to the window resizing and ajusts the canvas accordingly
			window.addEventListener("resize", e => {
				this.resize(e);
				if (this.cresize) this.cresize(e)
			})
		}
	...
	class Menu extends Process {
		constructor(m) {
			...
			this.back = {
				...
				data: () => {
					...
					this.currentMenu = this.history.shift();
					this.draw()
				}
			};
			this.init();
		...
		draw() {
			...
			objectsToDraw.forEach(v => {
				if (v.name.length > text.length) text = v.name
			});
			...
			this.ctx.font = fontsize + "px verdana";
			objectsToDraw.forEach(v => {
				this.ctx.globalAlpha = v.data ? 1 : 0.6;
				this.ctx.fillStyle = "black";
		...
		register() {
			this.mouse.move = click => {
				let cursor = "default";
				if (click.x > this.back.x && click.x < this.back.x + this.back.width && click.y > this.back.y && click.y < this.back.y + this.back.height && this.history.length) {
					cursor = "pointer"
				}
				this.currentMenu.forEach(v => {
					if (click.x > v.x && click.x < v.x + v.width && click.y > v.y && click.y < v.y + v.height && v.data) {
						cursor = "pointer";						
					}
				})
	...
	class Game extends Process {
		...
		tick(t) {
			if (!this.r) return;
			if (this.keyboard.Escape) return this.stop();
			this.updateDT(t);
			if(this.player1)this.player1.update(this, ["w", "s"]);
			if(this.player2)this.player2.update(this, ["ArrowUp", "ArrowDown"]);
			if(this.ball)this.ball.update(this);
			if (this.ball.x < 0) {
				this.score.player2++
			...
			if (this.score.player1 >= 10 || this.score.player2 >= 10) {
				if (confirm(this.score.lastWinner.name + " Won")) {
					this.stop({
						Rematch: () => {
							this.reset();
							return true;
		...
		reset() {
			this.r = this.ball.m = this.player1.m = this.player2.m = true;
		...
		start() {
			...
			this.touch.move = t => {
				t.touches.forEach(b => {
					if (b.clientX > 0 && b.clientX < this.player1.width * 10) {
						this.player1.vy = b.clientY - (this.player1.y + this.player1.height / 2);
					} else if (b.clientX > this.player2.x - this.player2.width * 10 && b.clientX < innerWidth && this.player2.name != "AI") {
			...
			this.r = true;
			setTimeout(() => {
				this.ball.m = this.player1.m = this.player2.m = true;
			}, 1000)
		}
		stop(nm = GameMenu) {
			console.log(m);
			this.r = this.player1.m = this.player2.m = this.ball.m = false;
			m = new Menu(nm);
		}
		cresize() {
	...
	class onlineGame extends Game{
		constructor() {
			...
			let player = {
				width: (innerWidth / 50) / innerWidth,
				height: (innerHeight / 5) / innerHeight
			}
			...
			this.player2 = new Player({
				x: innerWidth - (player.width * innerWidth),
				width: player.width,
				height: player.height,
				name: "Opponent",
				d: 0.8
			...
			this.sendObj;
			this.sendCommand;
			this.id = localStorage.id || null;
			this.ws.onopen = () => {
				//this.send = this.ws.send;
				this.sendObj = j => this.ws.send(JSON.stringify(j));
				this.sendCommand = (c, o = {}) => {
					o.command = c;
					this.sendObj(o);
				}
			}
		...
		tick(t){
			super.tick(t);
			this.send("update", {data: this.player1});
		}

		async send(identifier, data = {}){
			if(!identifier || !(this.ws.readyState == this.ws.OPEN))return;
			this.ws.send(
				JSON.stringify(
					Object.assign(
					{
		...
		async update(data){
			if(!data)return;
			let obj = data.data;
			this.player2.y = obj.p2.y * innerHeight;
			this.ball.x = obj.b.x * innerWidth;
			this.ball.y = obj.b.y * innerHeight;
		...
		async message(m) {
			...
			switch (o.command) {
				...
				case "queryId":
					if(this.id)
						this.send("assignId", {id: this.id});
					else
						this.send("queryId")
				break;
				case "refused":
					if (o.reason) throw new Error(o.reason);
					else throw new Error("Some unknown Error occured. try again later");
				break;
				case "assignId":
					let id = o.id;
					console.log("ID has been Assigned to " + id);
					if(!id)return;
					this.id = localStorage.id = id;
				break;
				case "pairing":
					console.log("Pairing")
				break;
				case "paired":
					console.log("Paired");
					this.send("update", {data: {vy: 20, y: 0.45}});
				break;
